...
const PricingPage = () => {
    return (
        <Section>
            {data.map((item) => {
                return <ContentWrapper key={item.id}>
                    <div>
                        <h3>{item.title}</h3>
                    </div>

                    <p>  <BsDot /> {item.subTitleOne}</p>


                    <p>  <BsDot /> {item.subTitleTwo}</p>
                    <p>  <BsDot /> Цена - {item.price} лв.</p>
                </ContentWrapper>
            })}
            <div style={{ width: '65%', margin: '0 auto', marginBottom: '2rem' }}>
                <h6 style={{ letterSpacing: '0.3rem', color: '#666' }}>
                    МОЖЕТЕ ДА ПОДАРИТЕ ВСЕКИ ЕДИН ОТ ПАКЕТИТЕ ПОД ФОРМАТА НА ВАУЧЕР.
                    ЗА ПОВЕЧЕ ИНФО ТУК:
                </h6>
            </div>
            <div style={{ marginBottom: '3rem', color: '#666' }}>
                <p>  <BsDot /> В избрания от Вас пакет <strong>НЕ </strong>са включени локация за снимки, професионален грим и прическа.</p>
                <p>  <BsDot /> Време за снимки –  от 2 до 4 часа</p>
                <p>  <BsDot /> Цените са за проекти с некомерсиални цели.</p>
                <p> <BsDot /> Цените са за проекти с некомерсиални цели.</p>
            </div>
        </Section>
    )
